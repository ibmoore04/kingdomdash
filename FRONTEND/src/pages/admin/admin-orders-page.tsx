...
import { getOrders, getOrderDetails, getPersonalShopperRequests } from '../../services/supabase/admin';
...
export const AdminOrdersPage: React.FC = () => {
  ...
  const loadOrders = useCallback(async () => {
    try {
      ...
      const shopperRows = (shopperRes.data || []).map((r) => ({
        id: r.id,
        order_number: `SHOP-${r.id.slice(0, 6).toUpperCase()}`,
        status: r.status,
        service_type: 'custom' as any,
      ...
      const mergedAll = [...standardRows, ...filteredShoppers].sort(
