...
import {
  ...
  type PlatformSettings as AdminSettingsState,
...
export function AdminSettingsTab() {
  ...
  const [settings, setSettings] = useState<AdminSettingsState>(DEFAULT_PLATFORM_SETTINGS)
  const [isSaving, setIsSaving] = useState(false)

  useEffect(() => {
    let isMounted = true
    fetchPlatformSettings().then((res) => {
  ...
  const handleToggle = (key: keyof AdminSettingsState) => {
    setSettings((prev) => {
      ...
      updatePlatformSettings(next).catch((err) => {
        console.error('Failed to sync setting toggle to Supabase:', err)
      })
      return next
    })
    pushToast({
      title: 'Admin Setting Updated',
      message: 'Platform configuration updated in database.',
      variant: 'success',
    })
  }
    ...
    try {
      ...
      if (res.success) {
        pushToast({
          ...
          message: 'Platform parameters and pricing matrix have been persisted to Supabase.',
          variant: 'success',
        })
      } else {
        pushToast({
          title: 'Save Failed',
          message: res.error || 'Could not write admin configuration.',
          variant: 'error',
        })
      }
    } catch (err: any) {
      pushToast({
        title: 'Save Failed',
        message: err?.message || 'Could not write admin configuration.',
        variant: 'error',
      })
    } finally {
      setIsSaving(false)
