...
export function Logo({ inverted = false, className, size = 'md' }: LogoProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-center rounded-lg bg-black px-1.5 py-1 shrink-0 overflow-hidden shadow-xs border border-white/10">
        <img
          src="/KingdomDash-logo.jpg"
          alt="KingdomDash"
          className="w-auto object-contain shrink-0"
          style={{ height: `${imgHeight}px`, maxHeight: `${imgHeight}px` }}
          width={Math.round(imgHeight * (1042 / 639))}
          height={imgHeight}
        />
      </div>
