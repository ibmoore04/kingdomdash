...
export const VendorSupportTab: React.FC = () => {
  ...
  const loadVendorTickets = useCallback(async () => {
    setIsLoading(true)
    try {
      const { data } = await getUserSupportTickets(profile?.email)
      setTickets(data || [])
    } catch {
      setTickets([])
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-border shadow-xs">
        <div>
          <h2 className="text-xl font-extrabold text-text-primary flex items-center gap-2">
            ...
            <span>Merchant & Vendor Support Desk</span>
          </h2>
          <p className="text-xs text-text-secondary mt-1">
            Submit partner inquiries, track menu/billing issues by Reference Code, or view admin replies.
          </p>
        </div>
        <Button asChild variant="primary" className="rounded-xl px-5 font-bold text-xs gap-2 shrink-0">
          <Link to="/support?reason=vendor_inquiry">
            <Plus className="w-4 h-4" />
            <span>Submit Vendor Ticket</span>
          </Link>
        </Button>
      </div>
      ...
      <div className="bg-gradient-to-r from-primary/5 via-white to-primary/5 p-6 rounded-2xl border border-primary/20 shadow-xs space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-text-primary">Track Report by Reference Number</h3>
        </div>
      ...
      {/* ── 2. VENDOR SUPPORT HISTORY ────────────────────────────────────── */}
      <div className="bg-white rounded-2xl border border-border p-6 shadow-xs space-y-4">
        <div className="flex items-center justify-between border-b border-border pb-4">
          <h3 className="text-base font-bold text-text-primary">Merchant Support History</h3>
          <Button
            variant="ghost"
            size="sm"
            onClick={loadVendorTickets}
        ...
        {isLoading ? (
          <div className="py-12 text-center text-xs text-text-muted">
            Loading merchant support tickets...
          </div>
        ) : tickets.length === 0 ? (
          <div className="py-12 text-center space-y-3">
            <HelpCircle className="w-10 h-10 text-neutral-300 mx-auto" />
            <div className="space-y-1">
              <p className="text-sm font-bold text-text-primary">No Merchant Support Tickets Logged</p>
              <p className="text-xs text-text-muted max-w-sm mx-auto">
                You haven&apos;t submitted any support requests under {profile?.email || 'your email'}.
              </p>
            </div>
            <Button asChild variant="outline" size="sm" className="rounded-xl font-bold text-xs">
              <Link to="/support?reason=vendor_inquiry">Submit Vendor Ticket</Link>
            </Button>
          </div>
        ) : (
